...
> ## ⚠️ Vendored chart — do not hand-edit
>
> Unlike the other charts here, this one is **not** built on the
> [`common`](../common) library. It is **vendored verbatim from upstream** so it
> can be served from this repository's Helm index. Upstream models a
> two-container pod (the `od` daemon + an nginx proxy sidecar) with a ConfigMap
> and Secret, which `common` does not cover — and it encodes app constraints
> (SQLite ⇒ single replica, SSE ⇒ proxy buffering off) worth keeping as-is.
>
> | | |
> |---|---|
